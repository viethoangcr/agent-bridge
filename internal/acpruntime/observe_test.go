...
// TestObserveExit_LeavesZombieBeforeGroupKill proves the runtime waiter's
// no-reap contract: observeExit reports a terminated direct child while it is
// still waitable, so its PID is still owned by its zombie and cannot be reused
// before the negative-PGID SIGKILL. cmd.Wait afterwards reaps the child and
...
// is not an unreaped child (ECHILD) cannot be observed, so the waiter keeps the
// reap-then-group-kill order.
