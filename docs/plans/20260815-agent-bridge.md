...
- `AGENT_BRIDGE_TOKEN` unset: no auth only on loopback. Set: every `/v1/*` route, including health, requires `Authorization: Bearer <token>`. Compare `sha256.Sum256` digests with `subtle.ConstantTimeCompare` so comparison time does not disclose token length. `GET /` remains public.
...
- Synthetic notifications are `_adapter/agent_exited` and `_adapter/invalid_stdout`; malformed, non-object, oversized, or invalid-UTF-8 agent stdout produces `_adapter/invalid_stdout` rather than persisting raw invalid bytes.

### ACP lifecycle and persistence

- One subprocess exists per live server ID and multiplexes ACP sessions itself. At most 64 ACP runtimes may be live; proxy admission reserves capacity atomically before durable row creation or spawn, returns 429 when full, and releases on every spawn failure or runtime exit. A runtime survives HTTP/SSE disconnects until DELETE, exit, bridge shutdown, or idle reap.
...
- Sequence columns use SQLite signed `INTEGER` and must never exceed `math.MaxInt64`; allocation rejects exhaustion. Foreign keys cascade; events index `(server_id,session_id,seq)`. Pin `modernc.org/sqlite` v1.57.0. One bridge owns one DB; use one SQL connection, WAL, foreign keys, 5s busy timeout, transactional sequence allocation, and checkpoint on shutdown. Construct the DSN as an escaped `file:` URI with `net/url` path and query APIs, never by concatenating the database path, so `?`, `#`, and `%` remain filename bytes. Set connection-state pragmas through v1.57.0's documented repeated `_pragma=` parameters (`journal_mode(WAL)`, `foreign_keys(1)`, `busy_timeout(5000)`, `synchronous(FULL)`) plus `_txlock=immediate`, never by `Exec` on one connection: `database/sql` recycles connections and would lose per-connection pragmas. `synchronous(FULL)` matches the commit-before-delivery durability contract.
...
- Process input uses separate wire and decoded limits: bound JSON to `min(10MiB, 4*ceil(maxInputBytesPerRequest/3)+1KiB)` and always enforce the decoded limit after base64/UTF-8 decoding. Each process permits one active context-aware stdin write; every write has a fixed server-owned five-second deadline. A timeout before any bytes rejects safely, while a timeout/error after a partial write SIGKILLs the process group.
...
- Phase 06 adds CI ownership for unit, race, static-build, keyless Docker E2E, and multiarchitecture verification; authenticated real-agent prompt/resume tests remain deferred until credentials exist.
- CI uses read-only `gofmt -l` drift detection. Third-party actions are pinned to full 40-character commit SHAs. `pull_request` and pushes to `main` run unit/vet/race/static and token-authenticated keyless host-architecture Docker E2E; non-publishing multiarchitecture verification runs on pushes to `main`, a weekly schedule, and `workflow_dispatch`.
