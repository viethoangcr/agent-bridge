...
- Runtime event notification is a capacity-one non-blocking/coalesced wakeup, never an event queue. SQLite is authoritative. SSE subscribes before reading a DB watermark, replays through it, then queries after wakeups or a bounded fallback ticker; dropped wakeups and lag cannot create gaps or duplicates.
