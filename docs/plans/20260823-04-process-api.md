...
|---|---|
| `internal/process/manager.go` | `manager.go`, `start.go`, `records.go` |
| `internal/process/run.go` | `run.go` with `prepareRun`/`spawnRun`/`awaitRun`/`finishRun` and the named `runPeakMultiplier` |
| `internal/process/config_test.go` | `config_test.go`, `json_test.go` |
| `internal/process/run_test.go` | `run_test.go`, `run_output_test.go` |
| `internal/httpapi/process.go` | `process_routes.go`, `process_handlers.go`, `process_input.go`, `process_query.go`, `process_error.go` |
...
- Fallback waits (no unreaped observation) switch the signal gate to direct-child-only before reaping, so external stop/kill can never target a reused PGID; the Linux path keeps SIGTERM/SIGKILL to the process group.
- Reservation invariant tests were added (the underflow clamps are unchanged).
